...
StringRef llvm::codeview::getSymbolLinkageName(CVSymbol Sym) {
  // Procedure symbols may have a linkage name (second string) after the
  // display name. This applies to:
  // - S_GPROC32_ID/S_LPROC32_ID symbols (always have both names when PSB is
  //   enabled)
  ...
  case SymbolKind::S_LPROC32_DPC_ID: {
    // Proc symbols have:
    // [fixed fields at offset 0-34][DisplayName\0][LinkageName\0] For non-_ID
    // symbols, there may be only DisplayName\0
    constexpr int NameOffset = 35;
    StringRef Content = toStringRef(Sym.content());
    ...
    // If the linkage name is empty, fall back to display name
    if (Result.empty())
      return getSymbolName(Sym);
