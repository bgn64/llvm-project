...
void ObjFile::initializeDependencies() {
  ...
  // Initialize PDB TpiSource from .debug$P/.debug$T sections.
  initializeTpiSource(ctx, /*forPsb=*/false);

  // Initialize PSB TpiSource from .psb$P/.psb$T sections (separate from PDB).
  initializeTpiSource(ctx, /*forPsb=*/true);
}
...
void ObjFile::initializeTpiSource(COFFLinkerContext &ctx, bool forPsb) {
  ...
  // Check for PCH sections first, then type sections.
  ArrayRef<uint8_t> data = getDebugSection(pchSection);
  if (!data.empty()) {
    isPCH = true;
  } else {
    data = getDebugSection(typeSection);
  }

  // Get the appropriate debugTypesObj and debugTypes fields.
  ...
  // Remember the type section data.
  types = data;
